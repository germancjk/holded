    ...
    submit: function submit(e) {
      ...
      if (this.messageError.length === 0) {
        axios.post('api/item', params).then(function (response) {
          _this.itemCreated = response.data.data; // add new sku

          _this.skus.forEach(function (element) {
            if (element.name.length > 0) {
              var paramsSku = {
                item_id: _this.itemCreated.id,
                name: element.name,
                cost: element.cost,
                sale_price: element.sale_price
              };
              axios.post('api/itemsku', paramsSku).then(function (response) {//
              });
            }
          });
        }); // clean objects
